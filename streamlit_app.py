...
# Run this script in Google Colab: exec(open('Streamlit_App.py').read())
# OR just run each section manually
...
subprocess.run([sys.executable, '-m', 'pip', 'install', 'streamlit==1.35.0', '-q'])
...
# ─── STEP 3: Download & merge Beijing Air Quality Dataset ─────────────────────
import pandas as pd
...
if not os.path.exists(DATA_PATH):
    ...
    if uploaded is not None:
        ...
        st.success("Dataset uploaded! Click any page in the sidebar to begin.")
else:
    ...
    if st.button("🗑️ Remove dataset and upload a new one"):
        ...
        st.rerun()
''')
print('Written: beijing_app/app.py')

# ─── STEP 5: Write 1_Dataset.py ──────────────────────────────────────────────
with open('beijing_app/pages/1_Dataset.py', 'w') as f:
    f.write('''import streamlit as st
import pandas as pd
import numpy as np
...
DATA_PATH = "beijing_merged_data.csv"
if not os.path.exists(DATA_PATH):
    st.error("Dataset not found. Please go to the Home page and upload it.")
    st.stop()
...
''')
print('Written: beijing_app/pages/1_Dataset.py')

# ─── STEP 6: Write 2_Visualisation.py ────────────────────────────────────────
with open('beijing_app/pages/2_Visualisation.py', 'w') as f:
    f.write('''import streamlit as st
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
...
with tab1:
    ...
    ax3.legend(fontsize=9); clean_ax(ax3)
...
with tab2:
    ...
    ax_h.set_title(f"Hour x Month PM2.5 — {hm_stn}")
...
with tab4:
    ...
    with col1:
        ...
        ax.set_ylabel(f"Mean {s_var}"); ax.legend(fontsize=8); clean_ax(ax)
        st.pyplot(fig, use_container_width=True); plt.close()
    with col2:
        ...
        ax2.set_ylabel(f"Mean {s_var}"); ax2.legend(fontsize=9); clean_ax(ax2)
        st.pyplot(fig2, use_container_width=True); plt.close()
''')
print('Written: beijing_app/pages/2_Visualisation.py')

# ─── STEP 7: Write 3_Model_Outputs.py ────────────────────────────────────────
with open('beijing_app/pages/3_Model_Outputs.py', 'w') as f:
    f.write('''import streamlit as st
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
...
with tab3:
    ...
    if submitted:
        ...
        st.info(f"{lvl}: {msg}")
''')
print('Written: beijing_app/pages/3_Model_Outputs.py')

# ─── STEP 8: Get IP and launch app ───────────────────────────────────────────
import urllib.request
ip = urllib.request.urlopen('https://ipv4.icanhazip.com').read().decode().strip()
print(f'\nYour IP (use as localtunnel password): {ip}')

subprocess.Popen(['streamlit', 'run', 'beijing_app/app.py'],
